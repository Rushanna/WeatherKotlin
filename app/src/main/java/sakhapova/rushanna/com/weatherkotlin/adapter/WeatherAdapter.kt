...
import sakhapova.rushanna.com.weatherkotlin.VIEW_TYPE_DAILY
import sakhapova.rushanna.com.weatherkotlin.VIEW_TYPE_NOW
...
class WeatherAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class CurrentViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun onBind(currentData: HourlyForecast, locationData: LocationData, simpleDateFormat: SimpleDateFormat) {
            ...
            tvTemperature.text =
                itemView.context.getString(R.string.temperature_format, currentData.temperature.toInt())
            tvFeelsLike.text = itemView.context.getString(
                R.string.temperature_feels_like_format,
                currentData.temperatureFeelsLike.toInt()
            )
            tvCityCountry.text = locationData.country + ", " + locationData.name
            tvDataTime.text = simpleDateFormat.format(Date())
            Picasso.with(itemView.context).load(currentData.condition.iconUrl.replace("//", "https://"))
                .into(conditionIv)
        }
    }
    ...
    class DayViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun onBind(
            ...
            dailyForecastContainer: DailyForecastContainer,
            dailyForecast: DailyForecast
        ) {
            tvData.text = simpleDateFormat.format(dailyForecastContainer.dateTs * 1000)
            tvTemperature.text =
                itemView.context.getString(R.string.temperature_format, dailyForecast.averageTemperature.toInt())
            Picasso.with(itemView.context).load(dailyForecast.condition.iconUrl.replace("//", "https://"))
                .into(ivCondition)
        }
    }
